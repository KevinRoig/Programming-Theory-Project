...
namespace EasyPrimitiveAnimals
{
    public class AnimalController : MonoBehaviour
    {
        ...
        //ENCAPSULATION
        private float m_movSpeed = 1f;
        public float movSpeed // Define speed that animal moves. This is also used to calculate leg movement speed.
        {
            get { return m_movSpeed; }
            set { m_movSpeed = value; }
        } 
        ...
        private void Start()
        {
            if (!this.gameObject.CompareTag("Chicken"))
            {
                ...
                rotSpeed = m_movSpeed * 4; // Set legs to move relative to animal moving speed.
            }
        }
        ...
        private void Update()
        {
            if (!this.gameObject.CompareTag("Chicken")) // If animal the script is attached to is not a chicken, animate 4 legs and body.
            {
                ...
                // Wander
                transform.Translate((Vector3.forward * Time.deltaTime) * m_movSpeed);
            } 
            else
            {
        ...
        {
            gameObject.GetComponent<Rigidbody>().AddForce(Vector3.up * 150f);
        }
